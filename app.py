...
# create-project :        create project name = selfservice - Prof of Concept server=[dev|stg|prd] site=[selfservice|default|mapbu] mode=[locked | managed]
#
...
#
# import-group :          import group name = Tableau-SelfServe-Automation-Consumers server=[dev|stg|prd] site=[selfservice|default|mapbu] role=[Explorer | Publisher]
#
...
## This driver function accept user input /query from slack app and perform action based on what is requested .
...
def serve_user_requests(message, say,client):
    ...
    query_dict = pquery.get_query_dict(user_query_kiv_str)
    ...
    tabapp = TableauApp(query_dict['server'],query_dict['site'],"PASS")
    ...
    #chek if the user entered command valid
    if pquery.is_valid_command(query_dict['cmd']):
        if tabapp.isValidServerAndSite:
            ...
            if query_dict['cmd'] == "create-project":
                ...
                if res:
                    ...
                    #tabapp.disconnect()
                    #print(f" time taken to create the project {duration}") 
                ...
                try:
                    tabapp.connect()
                    newgrp = tabapp.importGroup(query_dict['name'],role)
                    #keep it for debugging
                    end_time = datetime.now()
                    duration = end_time - start_time
            ...
            error = "STATUS:FAILED: Possibly, You have specified an Invalid Server Name or Site Name, Please check the spelling or query syntax"
            say(text=f"Hi <@{user_id}>  , {error} ", channel=user_id)
            tablogger.error(error,True)
      
       
    else:
        say(text=f"Thanks <@{user_id}> for using Tableau Self Service Slack bot, You entered an incorrect command !! For usage, please check the App Home Page Section Tab", channel=dm_channel)
        tablogger.error("STATUS:FAILED: Invalid Command Specified")
...
## This functionality provide additional options to perform some of the task using GUI - Two functionality implemented using this
...
def slack_user_command_dialog(body, client, ack, logger):
    ...
    configfile = parent_dir_path+"/library/"+"slackuserdialog.json"
    #print(configfile)
    try:
    ...
    if cmd_text.lower() in ap.valid_user_commands:
        ...
        try:
            ...
            print(f" the command is not yet implemented in the joson")
        #logger.debug(res)
...
## This will get called when the request will submit throgh GUI 
...
def dialog_submission_or_cancellation(ack: Ack, body: dict,say,client):
    ...
    user_query_kiv_str = pquery.get_user_dialog_data(body)
    ...
    tabapp = TableauApp(tserver,tsite)
    tserver_url = tabapp.serverInfo['TABLEAU_SERVER']
    ...
    if user_query_kiv_str['cmd'].lower() == "create-project":
        ...
        if res:
                    ...
                    try :
                        ...
                        error = "STATUS:PARTIAL SUCCESS Requested Project Created but Unable to make you Project Admin"
        ...
        try:
            tabapp.connect()
            newgrp = tabapp.importGroup(groupname,rolename)
...
##################
# Listen to the app_home_opened Events API  to hear when a user opens your app from the sidebar
# build the home Information 
